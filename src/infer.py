...
def extract_table_from_image(image_path: str | Path,
    ...
    if backend == "mlx":
        ...
        formatted = apply_chat_template(processor, config=None, prompt=prompt, num_images=1)
        return mlx_gen(model, processor, formatted, image=[str(image_path)],
...
@lru_cache(maxsize=2)
def load_llm(adapter: str | None = None):
    if _use_mlx():
        from mlx_lm import load as mlx_load
        model, tokenizer = mlx_load(C.STAGE2_LLM_MLX, adapter_path=adapter)
        return ("mlx", model, tokenizer)
    else:
        from transformers import AutoModelForCausalLM, AutoTokenizer
        import torch
        model = AutoModelForCausalLM.from_pretrained(
            C.STAGE2_LLM_HF, torch_dtype=torch.float16, device_map="auto"
        )
        tok = AutoTokenizer.from_pretrained(C.STAGE2_LLM_HF)
        if adapter:
            from peft import PeftModel
            model = PeftModel.from_pretrained(model, adapter)
        return ("hf", model, tok)
...
def chat(messages: list[dict], adapter: str | None = None,
    ...
    backend, model, tok = load_llm(adapter)
    if backend == "mlx":
        from mlx_lm import generate as mlx_gen
        prompt = tok.apply_chat_template(messages, add_generation_prompt=True, tokenize=False)
        return mlx_gen(model, tok, prompt=prompt, max_tokens=max_tokens, verbose=False)
    else:
        import torch
